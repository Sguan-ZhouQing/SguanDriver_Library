 ...
 * @LastEditTime: 2026-01-15 12:30:35
 * @FilePath: \SguanDriver库\【01】nRF24L01模块(SPI通信)\nRF24L01.c
 * @Description: 
 * 
...
{
    GPIO_CSN_SetBits(BitValue);
} 

void W_CE(uint8_t BitValue)
{
    GPIO_CE_SetBits(BitValue);
} 

uint8_t R_IRQ(void)
{
    return GPIO_IRQ_ReadBits();
}

uint8_t SPI_SwapByte(uint8_t Byte)
{
    uint8_t rxData;
    SPI_SwapData(&Byte, &rxDataY);
    return rxData;
}
